/** 單一版本的更新紀錄 */
export interface ChangelogEntry {
  /** 版號,遵循 SemVer(MAJOR.MINOR.PATCH) */
  version: string;
  /** 發布日期(YYYY-MM-DD) */
  date: string;
  /** 該版本的變更項目,依重要程度由上到下排列 */
  changes: string[];
}

export const CHANGELOG: ChangelogEntry[] = [
  {
    version: '1.4.0',
    date: '2026-09-09',
    changes: [
      '新增葬送的芙莉蓮合作、變身燃燒：露希妲、楓之谷學院的預設任務',
      '角色總覽重新設計為每日/每週/每月/賽季四張等重卡片，新增每月任務進度追蹤，並在當日到期時顯示提醒標記',
      '角色總覽新增 BOSS 討伐進度顯示，與收益分開呈現',
      '新增公告列，可在畫面上方顯示最新消息，支援手機版展開/收合',
      'BOSS 難度標籤色調整，加大難度間色相差異',
      '角色資訊的更新／刪除按鈕改為垂直排列',
      '修正深色/淺色模式下部分文字對比度不足的問題',
      '修正角色頁籤的無障礙標籤對應錯誤',
      'App 標題更名為「好楓寶進度追蹤器」',
      '任務清單新增「賽季」「單次」週期分類',
      '修正一次性任務後消失的問題',
      '新增任務、預設任務範本挑選畫面新增「賽季」重置週期選項',
      '角色總覽的賽季卡片新增任務進度顯示，與 BOSS 進度分開呈現',
      '新增任務的預設範本，新上架的分類會顯示 NEW 標籤並排到最上方',
      '備份/還原邏輯重構：刪除紀錄同步到所有裝置，不會被還原復活',
      '備份前先合併雲端上其他裝置的異動，不再整批覆蓋',
      '備份資料過時（超過一個月未更新）時會先跳出確認提醒使用者',
      '調整周末活動任務排序在每月任務前',
    ],
  },
  {
    version: '1.3.2',
    date: '2026-09-05',
    changes: [
      '修正特定時間查詢角色資料時，無法查詢的問題',
      '修正翻譯導致畫面當機的問題',
    ],
  },
  {
    version: '1.3.1',
    date: '2026-08-19',
    changes: [
      'BOSS 清單難度標籤依難度分級上色，一眼辨識難度',
      'BOSS 清單多人均分時，收益金額顯示「(每人)」標示',
      'BOSS 清單互動優化：修正欄位對齊、調整按鈕可點擊範圍',
      '修正 BOSS 清單排版在部分螢幕寬度下跑版的問題',
    ],
  },
  {
    version: '1.3.0',
    date: '2026-08-19',
    changes: [
      'BOSS 清單新增「攻略人數」設定，收益金額會依人數自動平分',
      '手機版任務清單/BOSS清單改為切換顯示',
      '電腦版任務清單/BOSS清單樣式優化，標題列固定不消失',
      '角色資訊排版優化',
      '自訂任務名稱字數上限及分類名稱上限調整',
    ],
  },
  {
    version: '1.2.1',
    date: '2026-08-13',
    changes: ['BOSS 清單新增「燦爛的凶星」、「尤比太」'],
  },
  {
    version: '1.2.0',
    date: '2026-08-13',
    changes: [
      '新增「更新／編輯角色資料」按鈕，可重新查詢或手動修改基本資料',
      'BOSS 清單新增重置倒數時間顯示',
      '每月任務新增「冠軍戰場」預設範本',
      '新增頁尾，可查看版本更新紀錄及 Github',
      '修正新增任務時，選擇分類或重置週期後對話框有時會意外關閉的問題',
    ],
  },
  {
    version: '1.1.0',
    date: '2026-08-11',
    changes: [
      '任務清單新增「每月」重置週期，可自訂建立每月任務',
      '任務清單新增「派對樂園」並提供快速建立範本',
      'BOSS 清單各週期區塊新增「全部完成」按鈕',
      '新增頁尾，顯示目前版本並可查看版本更新紀錄',
    ],
  },
  {
    version: '1.0.0',
    date: '2026-07-17',
    changes: [
      '支援建立多個角色，並各自追蹤每日/每週代辦任務與完成進度',
      '新增 BOSS 討伐清單，可依難度計算水晶收益並依週期分區顯示',
      '任務/BOSS 清單支援完成狀態篩選與分類區塊收合',
      '新增角色時可透過 NEXON Open API 查詢角色資料自動帶入',
      '新增資料管理頁面，支援 Google Drive 與本機檔案備份/還原',
      '支援淺色/深色模式切換',
    ],
  },
];

/** 目前應用程式版本,取自 CHANGELOG 最新一筆 */
export const APP_VERSION = CHANGELOG[0].version;