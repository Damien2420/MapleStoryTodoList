 ...
 *
 * 每日=楓橘、每週=森綠、每月=金黃、賽季=中性、週末活動=紫、單次=石板灰,顏色定義在 index.css 的 --cycle-* token,
 * 深淺主題各自調校。TaskList 與 BossList 的區塊標題 Badge 共用這份對照表。
 */
export const CYCLE_BADGE_CLASSES: Record<'每日' | '每週' | '每月' | '賽季' | '週末活動' | '單次' | 'VIP重置', string> = {
  ...
  週末活動: 'border-transparent bg-cycle-weekend text-cycle-weekend-foreground',
  單次: 'border-transparent bg-cycle-once text-cycle-once-foreground',
  VIP重置: 'border-transparent bg-cycle-vip text-cycle-vip-foreground',
};
